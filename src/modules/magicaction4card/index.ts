...
import { showHUD, HUDController, openUrl } from "@/utils/common"
...
import { makeThingsTask } from "../export2obsidian/utils"
...
export default defineConfig({
  ...
  actions4card: [
    ...
    {
      type: CellViewType.Button,
      key: "exportCard2Things",
      label: "导出到 Things",
      help: "请先选择一个卡片，然后点击此按钮。",
      method: async ({ nodes }) => {
        if (nodes.length < 1) {
          showHUD(hud.none_card);
          return;
        ...
        let allTexts:string[] = [];
        for (const node of nodes) {
          const exportTexts:string[] = makeThingsTask(node);
          allTexts = allTexts.concat(exportTexts);
        }
        const title = nodes[0].noteTitle;
        const items = allTexts.map(content => ({
            type: 'to-do',
            attributes: {
              title: `${content}`,
              when: 'someday',
        ...
        openUrl(`things:///json?data=${dataJson}`);
      }
    },
  ]
})
