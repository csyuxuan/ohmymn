...
export function makeThingsTask(
  node: MbBookNote,
): string[] {
  ...
  for (let i = 0; i < parentNotes.length; i++) {
    if (i == 0) {
      ...
      if (newNote && newNote.noteTitle) {
          res.push(newNote.noteTitle);
      }
    } else {
      if (parentNotes[parentNotes.length - i - 1].noteTitle) {
        res.push(parentNotes[parentNotes.length - i - 1].noteTitle);
      }
    }
  }

  if (node.noteTitle) {
    res.push(node.noteTitle);
  }

  const { onlyChildren } = getNodeTree(node)
  for (let i = 0; i < onlyChildren.length; i++) {
    if (onlyChildren[i].noteTitle) {
      res.push(onlyChildren[i].noteTitle);
    }
  }
